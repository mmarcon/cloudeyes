...
HTTPChannel.prototype.log = function(_){
    if(this.debug) {
...
var getACKManager = function(uuid, recipientId, context){
    return function ensureACK(error, request, response) {
        if(error) {
            return this.emit(Events.ERROR, response);
        }
        ...
        if(status === Message.status.OK) {
            if(type === Message.type.ACK) {
                return this.emit(Events.ACK, response);
            } else if(type === Message.type.NACK) {
                return this.emit(Events.NACK, response);
            }
        }
        return this.emit(Events.ERROR, response);
    }.bind(context);
